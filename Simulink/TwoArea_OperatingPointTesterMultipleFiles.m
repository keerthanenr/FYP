...
% H2=5000;
% filename = 'xpoly.xlsx';
% fulltestresults = xlsread(filename,'Sheet1');
k=1;
i=1;
M=zeros(10000,5);
j=0;
str1 = ["deltaf1smalltest","deltaf1smallquadtest", "deltaf2smalltest" ,"deltaf2smallquadtest"];
len_str=length(str1);
i=0;
while j<5
    r=1;
    ...
    filename=str1(j);
    fulltestresults = xlsread(filename,'Sheet1');
    while r<length(fulltestresults)+1
        H1=fulltestresults(r,2);
        H2=fulltestresults(r,1);
        PFR1=fulltestresults(r,3);
        PFR2=fulltestresults(r,4);
